...
import com.zipcodewilmington.froilansfarm.edibles.GerericVeggie;
...
public class DayTest {
    ...
    public void dailyRidesAndFeeding() {
        farm.getStableList().stream()
                ...
                .forEach(horse -> {
                    froilan.mount((Ridable) horse);
                    Assert.assertEquals(horse, froilan.getRidingDevice());
                    froilan.dismount();
                    Assert.assertEquals(null, froilan.getRidingDevice());
                    for (int i = 0; i < 3; i++) {
                        Assert.assertEquals("Numnum! Corn! Yeehaw!",horse.eat(new EarCorn()));
                    }
                });
        froilan.mount((Ridable) new com.zipcodewilmington.froilansfarm.animals.Horse());
    }
    ...
    public void tuesdayHarvest(){
        ...
        int numGen = yield.stream().mapToInt(fruit -> (fruit instanceof GerericVeggie) ? 1 : 0).sum();
    ...
    public void wednesdayBuyNewHorse() {
        ...
        froilan.mount((Ridable) newHorse);
